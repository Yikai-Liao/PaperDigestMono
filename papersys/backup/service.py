...
class BackupService:
    ...
    def create_bundle(self) -> BackupBundle:
        ...
        logger.info(
            "Created backup bundle {bundle} with {files} files ({size:.2f} KiB)",
            bundle=bundle.path.name,
            files=bundle.manifest["stats"]["files"],
            size=bundle.manifest["stats"]["bytes"] / 1024,
        )
        return bundle
    ...
    def run(self) -> BackupResult | None:
        ...
        try:
            ...
            logger.exception("Backup upload failed; removing staging artifact {bundle}", bundle=bundle.path)
            self._cleanup_bundle(bundle)
            raise

        logger.info(
            "Backup run finished: {files} files ({size:.2f} KiB) -> {uri}",
            files=bundle.manifest["stats"]["files"],
            size=bundle.manifest["stats"]["bytes"] / 1024,
            uri=remote_uri,
        )
